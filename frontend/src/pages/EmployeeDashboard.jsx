...
const WorkProgressBar = ({ settings, todayAttendance }) => {
  ...
  return (
    <div className="w-full mt-4 bg-gray-50 p-4 border border-gray-200 rounded-xl select-none pointer-events-none">
      ...
      <p className="text-[10px] text-gray-400 font-medium text-center italic mt-1">Updates live every 10 seconds</p>
    </div>
  );
};
  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-10">
      {/* Banner */}
      <div className="relative overflow-hidden rounded-xl bg-white px-4 py-4 sm:px-6 sm:py-5 shadow-sm border border-gray-200">
        <div className="absolute top-0 right-0 -translate-y-8 translate-x-8 opacity-20 pointer-events-none">
          <svg width="140" height="140" viewBox="0 0 200 200" fill="none"><circle cx="100" cy="100" r="100" fill="#e0e7ff" /></svg>
        </div>
        <div className="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/4 opacity-20 pointer-events-none">
          <svg width="180" height="180" viewBox="0 0 200 200" fill="none"><circle cx="100" cy="100" r="100" fill="#f3f4f6" /></svg>
        </div>
        <div className="relative z-10 flex items-center justify-between gap-3">
          {/* User Info & Avatar Group */}
          <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
            <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-xl bg-indigo-100 flex items-center justify-center flex-shrink-0 border-2 border-white shadow-sm overflow-hidden">
              {user?.profilePicture ? (
                <img src={user.profilePicture} className="w-full h-full object-cover" alt="User" />
              ) : (
                <span className="text-indigo-800 font-black text-lg sm:text-xl">{user?.name?.charAt(0)?.toUpperCase()}</span>
              )}
            </div>
            <div className="min-w-0">
              <div className="hidden sm:inline-flex items-center gap-1.5 bg-indigo-100 px-2.5 py-0.5 rounded-full text-indigo-800 text-[9px] font-bold uppercase tracking-widest mb-1.5 border border-indigo-200/50">
                <Star className="w-2.5 h-2.5 text-indigo-600" fill="currentColor" />
                <span>Employee Dashboard</span>
              </div>
              <h1 className="text-lg sm:text-2xl font-black text-gray-800 tracking-tight leading-tight truncate">
                Hello, {user?.name?.split(' ')[0]}
              </h1>
              <p className="text-gray-500 text-[10px] sm:text-sm font-medium truncate sm:mt-0.5">Keep up the great work!</p>
            </div>
          </div>

          {/* Points Badge */}
          <div className="flex items-center gap-2 sm:gap-3 bg-white/80 backdrop-blur-sm border border-gray-200/60 px-2.5 py-1.5 sm:px-5 sm:py-3 rounded-xl sm:rounded-xl shadow-sm hover:shadow-md transition-all duration-300 flex-shrink-0">
            <div className="bg-indigo-100 p-1.5 sm:p-2 rounded-lg sm:rounded-xl">
              <Trophy className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-indigo-700" />
            </div>
            <div className="flex flex-col">
              <span className="hidden sm:block text-gray-500 text-[9px] font-bold uppercase tracking-widest leading-none mb-1">Total Points</span>
              <span className="text-lg sm:text-2xl font-black text-gray-800 tracking-tight leading-none">
                {user?.performanceScore || 0}
                <span className="sm:hidden text-[9px] text-gray-400 font-bold ml-1 uppercase">pts</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        {/* Check-In Card — Bottom on mobile, Left on desktop */}
        <div className="order-2 md:order-1 bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col items-center justify-center text-center">
          <div className="flex flex-row items-center gap-3 mb-4 sm:flex-col sm:mb-4">
            <div className="w-8 h-8 sm:w-16 sm:h-16 bg-indigo-100 text-indigo-800 rounded-full flex items-center justify-center shadow-inner border border-indigo-200/50">
              <Clock className="w-4 h-4 sm:w-8 sm:h-8" />
            </div>
            <h3 className="text-base sm:text-xl font-black text-gray-800 tracking-tight leading-none">Attendance Management</h3>
          </div>
          ...
          {todayStatus.isHoliday ? (
            ...
            <div className="w-full">
              {isCheckOutTimeCrossed ? (
                <div className="mt-2 p-8 bg-red-50/40 border border-dashed border-red-200 rounded-xl">
                ...
                <>
                  <p className="text-sm text-gray-500 mb-4 font-medium select-none pointer-events-none">Standard Check-In: <span className="text-gray-800 font-bold">{formatSimpleTime(settings?.checkInTime)}</span></p>
            ...
            <div className="bg-gray-200 text-gray-700 px-6 py-4 rounded-xl font-bold w-full uppercase tracking-wider text-sm shadow-inner border border-gray-300 select-none pointer-events-none">
            ...
            <div className="w-full">
              <div className={`text-sm p-3 rounded-xl border flex items-center justify-center font-bold shadow-sm select-none pointer-events-none ${
                attendanceStatus === 'PENDING_APPROVAL' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' :
              ...
              {attendanceStatus === 'PENDING_APPROVAL' ? (
                <div className="mt-6 p-8 bg-indigo-50/50 border border-dashed border-indigo-200 rounded-xl">
                  <div className="flex flex-col items-center gap-4 text-center">
                    <div className="relative">
                      <Clock className="w-12 h-12 text-indigo-600 animate-pulse" />
                      <div className="absolute -top-1 -right-1 w-4 h-4 bg-indigo-500 rounded-full border-2 border-white animate-bounce" />
                    </div>
                    <div>
                      <h4 className="font-black text-indigo-900 tracking-tight text-lg">Wait for Admin Approval</h4>
                      <p className="text-xs text-indigo-700/80 font-semibold mt-2 max-w-[260px] leading-relaxed">Your late check-in is currently under review by the supervisor.</p>
                      <div className="mt-4 inline-flex items-center gap-2 bg-indigo-100/50 px-3 py-1.5 rounded-full border border-indigo-200">
                        <span className="w-2 h-2 bg-indigo-500 rounded-full animate-ping" />
                        <span className="text-[10px] font-bold text-indigo-800 uppercase tracking-wider">Review in progress</span>
                      </div>
                    </div>
                  </div>
                </div>
              ) : attendanceStatus === 'LATE_REJECTED' ? (
                <div className="mt-6 p-8 bg-red-50/50 border border-dashed border-red-200 rounded-xl">
                  <div className="flex flex-col items-center gap-4 text-center">
                    <div className="bg-red-100 p-3 rounded-xl shadow-sm border border-red-200">
        ...
        {/* Team Leaderboard — Top on mobile, Right on desktop */}
        <div className="order-1 md:order-2 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
          <div className="px-4 py-3 sm:px-6 sm:py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3">
            <div className="flex items-center justify-between w-full sm:w-auto">
              <h3 className="text-sm sm:text-base font-bold text-gray-800 flex items-center gap-2 leading-none">
                <Trophy className="w-3.5 h-3.5 sm:w-4 h-4 text-indigo-600" /> Team Leaderboard
              </h3>
              <p className="sm:hidden text-[9px] text-gray-400 font-medium italic">Anonymous View</p>
            </div>
            {selfRank && (
              <div className="flex items-center justify-between w-full sm:w-auto bg-indigo-50 border border-indigo-200 rounded-lg sm:rounded-xl px-3 py-1.5 sm:px-3 sm:py-2 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <Crown className="w-3.5 h-3.5 text-indigo-600" />
                  <div className="flex items-baseline gap-1.5 sm:block">
                    <p className="text-[8px] sm:text-[9px] uppercase tracking-wider text-indigo-600 font-bold">Rank</p>
                    <p className="text-sm sm:text-base font-black text-indigo-700 leading-tight">#{selfRank}</p>
                  </div>
                </div>
                <div className="w-px h-4 sm:h-7 bg-indigo-200 mx-2 hidden sm:block" />
                <div className="flex items-baseline gap-1.5 sm:block text-right">
                  <p className="text-[8px] sm:text-[9px] uppercase tracking-wider text-indigo-600 font-bold">Score</p>
                  <p className="text-sm sm:text-base font-black text-indigo-700 leading-tight">{user?.performanceScore ?? 0}</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div 
            ...
            className="flex-1 overflow-y-auto scroll-smooth max-h-[230px] sm:max-h-[400px]"
          >
            {lbLoading ? (
              <div className="px-6 py-10 text-center text-gray-400 text-sm">Loading leaderboard...</div>
            ) : leaderboard.length === 0 ? (
              <div className="px-6 py-10 text-center text-gray-400 text-sm italic">No data yet.</div>
            ) : (
                    ...
                    >
                      <div className="w-8 flex-shrink-0 flex items-center justify-center">
                        {medal
                          ...
                          : <span className={`text-xs font-bold tabular-nums ${entry.isSelf ? 'text-indigo-600' : 'text-gray-400'}`}>#{entry.rank}</span>
                        }
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        {entry.isSelf
                          ...
                          : <p className="text-xs font-semibold text-gray-400 blur-[4px] select-none truncate">{entry.name}</p>
                        }
                        <p className="text-[9px] text-gray-400 font-medium">{entry.isSelf ? 'You' : 'Anonymous'}</p>
                      </div>
                      <div className="flex-1 max-w-[80px] hidden sm:block">
                        <div className="w-full bg-gray-100 rounded-full h-1 overflow-hidden">
                          <div
                            className={`h-1 rounded-full ${entry.isSelf ? 'bg-indigo-400' : isTop3 ? 'bg-gray-400' : 'bg-gray-300'}`}
                            style={{ width: `${Math.min(100, (entry.performanceScore / (leaderboard[0]?.performanceScore || 1)) * 100)}%` }}
                          />
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className={`text-xs font-extrabold tabular-nums ${entry.isSelf ? 'text-indigo-700' : 'text-gray-600'}`}>{entry.performanceScore}</p>
                        <p className="text-[8px] uppercase tracking-widest text-gray-400 font-semibold">pts</p>
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="px-5 py-2.5 border-t border-gray-100 bg-gray-50/50">
            <p className="text-[9px] text-gray-400 text-center italic">Names and avatars are hidden to protect privacy.</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          <h3 className="text-lg font-bold text-gray-800 flex items-center">
            <Activity className="w-5 h-5 text-indigo-800 mr-2" /> Performance Analytics
          </h3>
          <div className="flex bg-gray-100 p-1 rounded-lg mt-4 sm:mt-0 border border-gray-200">
            <button onClick={() => setChartFilter('weekly')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${chartFilter === 'weekly' ? 'bg-white shadow text-gray-800' : 'text-gray-600 hover:text-gray-800'}`}>Week</button>
            <button onClick={() => setChartFilter('monthly')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${chartFilter === 'monthly' ? 'bg-white shadow text-gray-800' : 'text-gray-600 hover:text-gray-800'}`}>Month</button>
            <button onClick={() => setChartFilter('yearly')} className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${chartFilter === 'yearly' ? 'bg-white shadow text-gray-800' : 'text-gray-600 hover:text-gray-800'}`}>Year</button>
          </div>
        </div>
          ...
          {chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <Tooltip
                  contentStyle={{ borderRadius: '12px', border: '1px solid #e5e7eb', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', backgroundColor: '#ffffff' }}
                  labelStyle={{ fontWeight: 'bold', color: '#1f2937' }}
                  itemStyle={{ color: '#4f46e5', fontWeight: 'bold' }}
                />
                <Area type="monotone" dataKey="netScore" stroke="#4f46e5" strokeWidth={3} fillOpacity={1} fill="url(#colorScore)" dot={{ r: 4, strokeWidth: 2, fill: '#fff' }} activeDot={{ r: 6 }} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="text-base font-bold text-gray-800 tracking-tight">Today's Snapshot</h3>
        ...
        {todayStatus.isHoliday ? (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {/* Status */}
            <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-2 p-3 bg-white rounded-xl border border-gray-200">
              <div className="flex items-center gap-2">
                {attendanceStatus === 'EARLY' ? <CheckCircle2 className="w-4 h-4 text-emerald-600" /> :
                  attendanceStatus === 'LATE' || attendanceStatus === 'LATE_REJECTED' ? <XCircle className="w-4 h-4 text-red-600" /> :
                    attendanceStatus === 'PENDING_APPROVAL' ? <AlertCircle className="w-4 h-4 text-indigo-600" /> :
            ...
            {/* Check-in */}
            <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-2 p-3 bg-white rounded-xl border border-gray-200 select-none pointer-events-none">
              <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-gray-800">{formatTime(todayAttendance.checkInTime)}</span>
            </div>
            ...
            <div className={`flex flex-col xs:flex-row xs:items-center justify-between gap-2 p-3 rounded-xl border ${pointsAwarded >= 0 ? 'bg-green-50 border-green-100' : 'bg-red-50 border-red-100'}`}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Points</span>
              </div>
          ...
          <div className="flex flex-col items-center justify-center text-center py-8 bg-gray-100/50 rounded-xl border border-dashed border-gray-300">
            <Clock className="w-8 h-8 text-gray-400 mb-2" />
            <p className="text-sm font-semibold text-gray-600">No Attendance Yet</p>
            <p className="text-xs text-gray-500 mt-1">Slide to check in and start tracking</p>
          </div>
        )}
      </div>
