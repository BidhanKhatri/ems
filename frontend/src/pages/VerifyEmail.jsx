...
const VerifyEmail = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If expired, allow resend even if timer hasn't finished
        ...
        <div className="px-8 pb-10">
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="flex justify-center gap-4">
              {otp.map((digit, index) => (
                <input
                  ...
                  onPaste={handlePaste}
                  className="w-14 h-16 text-center text-2xl font-bold bg-white/80 border border-gray-100 rounded-2xl focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none text-gray-900 shadow-sm"
                />
              ))}
            </div>
            ...
            <button
              ...
              className="w-full py-4 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold transition-all disabled:opacity-50 disabled:hover:bg-indigo-600 shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 group transform active:scale-[0.98]"
            >
              {loading ? (
          ...
          <div className="mt-8 pt-6 border-t border-gray-100/50 flex items-center justify-center">
             <Link to="/register" className="flex items-center gap-2 text-xs font-bold text-indigo-500 hover:text-indigo-700 transition-colors">
                <ArrowLeft size={14} />
